...
const inventoryAnalyzer = async (backlog, currentInventory, currentProducts) => {
  if (backlog.length < 1)
    return backlog
  const purchase_items = await knex('purchases_items').where({purchase_id: backlog[0]}).select('item_id', 'item_qty')
  const purchase_bundles = await knex('purchases_bundles').where({purchase_id: backlog[0]}).select('bundle_id', 'bundle_qty')
  console.log(purchase_items, currentProducts.items);
  purchase_items.map(items => {
    const itemMatch = currentProducts.items.find(function(itemId) {
      return itemId === purchase_items.id;
    })
    return itemMatch
  })
}
...
backlog.shift()
return inventoryAnalyzer(backlog, currentInventory, currentProducts)
}
